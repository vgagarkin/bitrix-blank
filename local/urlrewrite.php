    ...
    if (($p = strpos($_SERVER['QUERY_STRING'], $marker)) !== false) {
        ...
        if (($p = strpos($uri, '/', $p + strlen($marker))) !== false) {
            if ($_SERVER['REQUEST_URI'] === ''
                ...
                || strpos($_SERVER['REQUEST_URI'], $marker) !== false
            ) {
...
$requestUriWithoutParams = ($foundQMark !== false
    ? substr($_SERVER['REQUEST_URI'], 0, $foundQMark) : $_SERVER['REQUEST_URI']);
...
if (!defined('BX_UTF') && CUtil::DetectUTF8($_SERVER['REQUEST_URI'])) {
    $requestPage = \Bitrix\Main\Text\Encoding::convertEncoding(
        $requestPage,
        'utf-8',
        (defined('BX_DEFAULT_CHARSET') ? BX_DEFAULT_CHARSET : 'windows-1251')
    );
}
...
$arCustomUrlRewrite = array();

if (file_exists($_SERVER['DOCUMENT_ROOT'] . '/urlrewrite_custom.php')) {
    include $_SERVER['DOCUMENT_ROOT'] . '/urlrewrite_custom.php';
}
...
$arUrlRewrite = array_merge($arCustomUrlRewrite, $arUrlRewrite);

if (isset($_GET['SEF_APPLICATION_CUR_PAGE_URL'])
    || ((string)$_SERVER['REDIRECT_STATUS'] === '404'
        && isset($_SERVER['REDIRECT_STATUS']))
) {
    if (isset($_SERVER['REDIRECT_STATUS'])
        && $_SERVER['REDIRECT_STATUS'] === '404'
        && !isset($_GET['SEF_APPLICATION_CUR_PAGE_URL'])
    ) {
        $url = $requestUri;
    } else {
    ...
    if (isset($_GET['SEF_APPLICATION_CUR_PAGE_URL'])
        && (
            isset($_SERVER['REDIRECT_STATUS']) && (int)$_SERVER['REDIRECT_STATUS'] === 404
    ...
    define(
        ...
        htmlspecialcharsbx('/bitrix/urlrewrite.php?SEF_APPLICATION_CUR_PAGE_URL=' . urlencode($uriPath))
    );
}
    ...
    foreach ($arUrlRewrite as $val) {
        if (preg_match($val['CONDITION'], $requestUri)) {
            if (strlen($val['RULE']) > 0) {
                $url = preg_replace(
                    $val['CONDITION'],
                    (strlen($val['PATH']) > 0 ? $val['PATH'] . '?' : '') . $val['RULE'],
                    $requestUri
                );
            } else {
                $url = $val['PATH'];
            ...
            if ($urlTmp7 === 'upload/'
                || ($urlTmp7 === 'bitrix/'
                    && 0 !== strpos($urlTmp, 'bitrix/services/')
                    && 0 !== strpos($urlTmp, 'bitrix/groupdavphp')
                )
            ) {
                continue;
            }
